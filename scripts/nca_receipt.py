...
PULL   = float(os.getenv("PULL",   "0.26"))  # blueprint pull
SMOOTH = float(os.getenv("SMOOTH", "0.36"))  # local blur mix
COARSE = float(os.getenv("COARSE", "0.22"))  # coarse pull
...
def blur3(a: np.ndarray) -> np.ndarray:
    ...
    h, w = a.shape
    out = np.zeros_like(a)
    # manual conv (fast enough for 64x64)
    for dy in (-1,0,1):
        for dx in (-1,0,1):
...
def build_receipt(telem):
    ...
    because = [
        ...
        "Local smoothing + frozen blueprint (morphogen) + coarse pull drive healing",
