...
def unsupported_claim_ratio(text:str)->float:
    ...
    for i,t in enumerate(s):
        if SUPPORT.search(t) or CITEPAT.search(t):
            for j in (i-1,i,i+1):
                if 0<=j<len(s): supported_window.add(j)
...
def compute_kappa(length_tokens:int, mu_len:float, var_len:float,
    ...
    # centered stress: calm near 0, stress > 0 as rho outruns S*
...
DEFAULT_CFG = {
  "tau_k": 0.75,          # stress cutoff
  "tau_hol": 0.35,        # path drift cutoff (after EWMA)
  "ucr_min": 0.40,        # unsupported-claim ratio
  "es_min": 0.25          # evidence strength relief floor
}
...
def verdict(digest, telem, cfg):
    # hard fail on cycles
    if digest["cycle_plus"]>0: return "red"
    red =  (telem["kappa_eff"]>=cfg["tau_k"] and digest["ucr"]>=cfg["ucr_min"] and telem["evidence_strength"]<cfg["es_min"]) \
...
def main():
    # 1) get text (docs/input.txt or benign demo)
